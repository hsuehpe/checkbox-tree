...
import NodeModel from "./NodeModal";
...
const TreeView: FC<TreeViewProps> = ({
	...
	const handleCheck = useCallback(
		({ value, checked }: { value: string; checked: boolean }) => {
			...
			const serializedList = clonedModel.serializeList("checked");
	...
	const handleExpand = useCallback(
		({ value, expanded }: { value: string; expanded: boolean }) => {
			const clonedModel = model.clone();
			clonedModel.toggleNode(value, "expanded", expanded);

			const serializedList = clonedModel.serializeList("expanded");
